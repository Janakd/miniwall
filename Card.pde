class Card {
  ...
  void render() {
      //if the current time is bigger than "timerEnd"  
//      println("current time " + millis() + ". End time " + timerEnd) ;  
      if(isTimeUp(timerEnd)){
        println("time is up") ;
      }
      
      
    if (touchEvent == TOUCHING || (timerEnd!=0) && !isTimeUp(timerEnd)) {
      
      //glowPNG = loadImage("glow.png");
      
      fill(255);
      //rect(x, y, 100, 100);
      image(loadImage("glow_rect.png"), x, y);
      
      fill(255, 255, 255);
      textSize(20);
      text (desc, x-75, y-75);
      textSize(13);
      text (rewards, x-75, y+70, 170, 200);
  ...
  void countDown(int mil) {
    timerEnd = millis() + mil ;
    println("start a timer. timerEnd at " + timerEnd ); 
  }

  boolean isTimeUp( int end) {
//    println("current time " + millis() + ". End time " + end) ;  
    if (millis()>end && timerEnd!=0) {
      timerEnd = 0 ; //as a flag that timer isn't set yet 
      return true;
    }
    return false;
  }
